...
def get_attack(attack_name, classifier, eps=0.03, targeted=False, max_iter=10, nb_grads=10, eps_step=None, confidence=0.0, initial_const=0.1, binary_search_steps=5):
    if attack_name == 'FGSM':
        ...
        return DeepFool(classifier=classifier, max_iter=max_iter, nb_grads=nb_grads, verbose=False)
...
def denormalize_np(img, mean, std):
    return (img * np.array(std)[:, None, None]) + np.array(mean)[:, None, None]

def normalize_np(img, mean, std):
    return (img - np.array(mean)[:, None, None]) / np.array(std)[:, None, None]
...
def show_transformed_images(images, num=5, title='Transformed Images'):
...
def run_attack_on_dataset(model, dataloader, attack_type='FGSM', user_params={}, transform_defense=0):
    ...
    run_single_attack(classifier, dataloader, attack, transform_defense=transform_defense)
    print(f"\n[FINISHED] Attack: {attack_type}")
    print("[PARAMETERS]:")
    for k, v in user_params.items():
        print(f"  {k} = {v}")

def run_single_attack(classifier, data_loader, attack, transform_defense=0):
    all_preds_adv = []
    all_labels = []
    all_ssim = []
    all_psnr = []
    ...
    with tqdm(total=total_images, desc="Processing images") as pbar:
        for images, labels in data_loader:
            ...
            if transform_defense == 1:
                # Median filter
                adv_images_transformed = []
                for adv in adv_images:
                    adv_np = denormalize_np(adv, model_mean, model_std)
                    filtered = np.zeros_like(adv_np)
                    for c in range(3):
                        filtered[c] = median_filter(adv_np[c], size=3)
                    adv_images_transformed.append(normalize_np(filtered, model_mean, model_std))
                adv_images = np.stack(adv_images_transformed)

            elif transform_defense == 2:
                # Gaussian blur
                adv_images_transformed = []
                for adv in adv_images:
                    adv_np = denormalize_np(adv, model_mean, model_std)
                    blurred = gaussian_filter(adv_np, sigma=(0, 1, 1))
                    adv_images_transformed.append(normalize_np(blurred, model_mean, model_std))
                adv_images = np.stack(adv_images_transformed)

            elif transform_defense == 3:
                # JPEG compression
                adv_images_transformed = []
                for adv in adv_images:
                    adv_np = denormalize_np(adv, model_mean, model_std)
                    adv_np = (adv_np * 255).astype(np.uint8).transpose(1, 2, 0)
                    _, encimg = cv2.imencode('.jpg', adv_np, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
                    decimg = cv2.imdecode(encimg, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.0
                    if decimg.ndim != 3 or decimg.shape[2] != 3:
                        continue
                    decimg = decimg.transpose(2, 0, 1)
                    adv_images_transformed.append(normalize_np(decimg, model_mean, model_std))
                adv_images = np.stack(adv_images_transformed)

            adv_images = torch.tensor(adv_images, dtype=torch.float32).to(classifier._device)
...
if __name__ == '__main__':
    ...
    if model_class:
        ...
        print("\nChoose post-attack transformation (defense):")
        print("0 - None")
        print("1 - Median filter")
        print("2 - Gaussian blur")
        print("3 - JPEG compression")
        transform_choice = int(input("Your choice (0-3): "))
        ...
        for i, combo in enumerate(param_combos):
            ...
            run_attack_on_dataset(
                ...
                user_params=combo,
                transform_defense=transform_choice
            )
